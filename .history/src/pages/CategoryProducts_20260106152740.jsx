...
const CategoryProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const currentCategoryProducts = current.filter(
        (p) => p.subcategories.categories.slug === category
      );

      const otherCategoryProducts = current.filter(
        (p) => p.subcategories.categories.slug !== category
      );

      setProducts(currentCategoryProducts);
      setOtherProducts(otherCategoryProducts);
      setLoading(false);
    };
    ...
    // ✅ Products of CURRENT category (main grid)
  const currentCategoryProducts = products.filter(
    (product) => product.category === category
  );

  // ✅ Products of OTHER categories (slider)
  ...
  const filteredProducts =
    finish === "All"
      ? products
      : products.filter((p) =>
          p.product_finishes.some((f) => f.finish_name === finish)
        );
  ...
  return (
    ...
    {/* OTHER CATEGORY PRODUCTS */}
        ...
        <section className="pt-32 pb-24 bg-white">
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-gray-900 capitalize mb">
                Other Collections
            </h2>

            <ProductSlider products={otherCategoryProducts} />
            </div>
        </section>
        )}
    <CTC/>
