...
const SingleProduct = () => {
  ...
  useEffect(() => {
      const fetchProduct = async () => {
        ...
        // Process other products for ProductSlider
        const processedOthers = (others || []).map((p) => ({
          ...p,
          category: p.subcategories?.categories?.slug || "unknown",
          image: p.product_images?.find((img) => img.image_type === "product")?.image_url ||
                 p.product_images?.find((img) => img.image_type === "hero")?.image_url ||
                 "/placeholder.jpg",
          finish: p.product_finishes?.[0]?.finish_name || "—",
        }));
